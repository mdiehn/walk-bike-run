...
test('saves, loads, copies, and deletes routes in the local library', async ({ page }) => {
  await page.goto('/');
  ...
  await expect(page.getByTestId('save-status')).toHaveText('Saved in route library');
  await expect(page.getByTestId('saved-route-list')).toContainText('Library test walk');
  ...
  await expect(page.getByTestId('saved-route-list')).toContainText('Library test walk copy');
