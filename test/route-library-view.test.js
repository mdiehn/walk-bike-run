...
describe('route library view', () => {
  ...
  it('filters by route name', () => {
    expect(
      getVisibleSavedRoutes(savedRoutes, { nameFilter: 'apple' }).map(
        (route) => route.id,
      ),
    ).toEqual(['walk']);
  });

  it('filters by distance bucket', () => {
    expect(
      getVisibleSavedRoutes(savedRoutes, { distanceFilter: 'gte10' }).map(
        (route) => route.id,
      ),
    ).toEqual(['bike']);
  });

  it('filters by estimated duration bucket', () => {
    expect(
      getVisibleSavedRoutes(savedRoutes, { durationFilter: 'gte60' }).map(
        (route) => route.id,
      ),
    ).toEqual(['bike']);
  });
  ...
  it('can reverse name sorting', () => {
    expect(
      getVisibleSavedRoutes(savedRoutes, {
        sortBy: 'name',
        sortDirection: 'desc',
      }).map((route) => route.name),
    ).toEqual(['Zoo run', 'Bike loop', 'Apple walk']);
  });

  it('sorts by point count descending by default', () => {
    expect(
  ...
  it('sorts by created date descending by default', () => {
    expect(
      getVisibleSavedRoutes(savedRoutes, { sortBy: 'created' }).map(
        (route) => route.id,
      ),
